...
namespace pvpgn
{
	...
	namespace bnetd
	{
        ...
        static void add_ip_register_record(unsigned int ip) {
            ...
            eventlog(eventlog_level_info, __FUNCTION__, " Registered new account from IP: {}", addr_num_to_addr_str(ip, 0));
           ...
           // 2. 获取当前 IP 在清理过期记录前的总记录数
            ...
            eventlog(eventlog_level_info, __FUNCTION__, "Registered new account from IP: {}, total registrations (including expired): {}", addr_num_to_addr_str(ip, 0), count);
		...
		static int _client_createaccountw3(t_connection * c, t_packet const *const packet)
		{
            ...
            {
                eventlog(eventlog_level_info, __FUNCTION__, "[{}] IP 24小时内注册账号已超过5个，拒绝账号创建请求", aaddr_num_to_addr_str(ip, conn_get_port(c)));
		...
		static int _client_createacctreq1(t_connection * c, t_packet const *const packet)
		{
            ...
            if (!check_ip_register_limit(client_ip))
            {
                eventlog(eventlog_level_info, __FUNCTION__, "[{}] IP 24小时内注册账号已超过5个，拒绝账号创建请求", addr_num_to_addr_str(ip, conn_get_port(c)));
